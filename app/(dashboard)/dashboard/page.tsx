...
import { 
  ...
  Briefcase, FileText, Users, Mail, TrendingUp, Loader2, Send, HelpCircle 
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_BASE_API}/analytics/stats`);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-[70vh]">
        ...
        <p className="text-muted-foreground animate-pulse font-medium">Synchronizing dashboard data...</p>
      </div>
    );
  }
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Real-time performance and content metrics.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard label="Projects" value={stats?.projects} icon={Briefcase} color="bg-blue-500" />
        <StatCard label="Blogs" value={stats?.blogs} icon={FileText} color="bg-pink-500" />
        <StatCard label="Subscribers" value={stats?.subscribers} icon={Send} color="bg-indigo-500" />
        <StatCard label="Inquiries" value={stats?.messages} icon={Mail} color="bg-orange-500" />
      </div>
          ...
          <div className="h-87.5 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={chartData} margin={{ top: 20, right: 0, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} fontSize={10} fontWeight="bold" tick={{fill: '#94a3b8'}} dy={10} />
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900 p-6 rounded-4xl border border-slate-100 dark:border-slate-800 shadow-sm">
             ...
             <div className="grid grid-cols-2 gap-3">
                ...
                <QuickAction label="New Project" icon={Briefcase} href="/dashboard/create-project" />
                <QuickAction label="FAQs" icon={HelpCircle} href="/dashboard/faqs" />
